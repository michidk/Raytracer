...
namespace Raytracer
{
    public class Sphere : Renderable
    {
        ...
        public override RaycastHit HitObject(Ray ray, double nearPlane, double farPlane)
        {
            ...
            if (proj < 0)
                return
                    new RaycastHit(); // projected vector points to opposite direction and therefor can't hit the sphere

            var squaredD =
                dist.Dot(dist) -
                Math.Pow(proj,
                    2); // squared length from sphere center to hit point on raycast direction (orthogonal to raycast dir)
            ...
            var innerT =
                Math.Sqrt(squaredRadius - squaredD); // distance from hit point to center projected on raycast dir
